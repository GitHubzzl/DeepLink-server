...
let moduleSQL = {
    ...
    getModuleInfoByPath:`SELECT m.*, IF ( m.module_type = 0, fa.folder_type_name, ea.element_type_name ) AS module_type_name, IF ( m.module_type = 0, ea.folder_type_name, eb.element_type_name ) AS parent_type_name FROM module_list AS m LEFT JOIN folder_type_dic AS fa ON fa.folder_type_id = m.module_type_id LEFT JOIN folder_type_dic AS fb ON fb.folder_type_id = m.parent_type_id LEFT JOIN element_type_dic AS ea ON ea.element_type_id = m.module_type_id LEFT JOIN element_type_dic AS eb ON eb.element_type_id = m.parent_type_id WHERE path =?`,
    getModuleInfoByPathId:`SELECT m.*, IF ( m.module_type = 0, fa.folder_type_name, ea.element_type_name ) AS module_type_name, IF ( m.module_type = 0, fa.folder_type_name, eb.element_type_name ) AS parent_type_name FROM module_list AS m LEFT JOIN folder_type_dic AS fa ON fa.folder_type_id = m.module_type_id LEFT JOIN folder_type_dic AS fb ON fb.folder_type_id = m.parent_type_id LEFT JOIN element_type_dic AS ea ON ea.element_type_id = m.module_type_id LEFT JOIN element_type_dic AS eb ON eb.element_type_id = m.parent_type_id WHERE path_id = ?`,
